...
<style>
    ...
    .stSelectbox label, .stSlider label, .stNumberInput label { color: #ccd6f6 !important; font-weight: 500 !important; }
    ...
    .stSelectbox > div > div:hover, .stNumberInput > div > div > input:hover { border-color: #4a6cf7 !important; }
    .stSlider > div > div > div { background-color: #2a2a4a !important; }
    .stSlider > div > div > div > div { background-color: #4a6cf7 !important; }
    .stButton > button {
    ...
    .stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 30px rgba(74,108,247,0.5) !important; }
    .prediction-card {
    ...
    .prediction-card:hover { transform: translateY(-5px); border-color: #4a6cf7; }
    .prediction-value { font-size: 3rem; font-weight: 700; margin: 0.5rem 0; }
    .prediction-label { font-size: 0.9rem; color: #8892b0; text-transform: uppercase; letter-spacing: 0.05em; }
    .risk-high { color: #ff6b6b; background: linear-gradient(135deg, #ff6b6b, #ee5a24); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .risk-medium { color: #ff9f43; background: linear-gradient(135deg, #ff9f43, #f0932b); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .risk-low { color: #4a6cf7; background: linear-gradient(135deg, #4a6cf7, #6a4cf7); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .recommendation-item {
    ...
    .recommendation-item:hover { border-left-color: #6a4cf7; background: linear-gradient(145deg, #1a1a2e, #16213e); }
    ...
    .risk-indicator:hover { transform: scale(1.02); }
    .risk-indicator-high { background: rgba(255,107,107,0.15); border: 1px solid rgba(255,107,107,0.3); }
    .risk-indicator-medium { background: rgba(255,159,67,0.15); border: 1px solid rgba(255,159,67,0.3); }
    .risk-indicator-low { background: rgba(74,108,247,0.15); border: 1px solid rgba(74,108,247,0.3); }
</style>
...
with col_right:
    ...
    st.markdown("""
    <div class="risk-indicator risk-indicator-high"><span style="font-size:1.2rem;">🔴</span> <strong style="color:#ff6b6b;">High Risk</strong> <span style="font-size:0.8rem;color:#8892b0;">>70%</span></div>
    <div class="risk-indicator risk-indicator-medium"><span style="font-size:1.2rem;">🟡</span> <strong style="color:#ff9f43;">Medium Risk</strong> <span style="font-size:0.8rem;color:#8892b0;">40-70%</span></div>
    <div class="risk-indicator risk-indicator-low"><span style="font-size:1.2rem;">🟢</span> <strong style="color:#4a6cf7;">Low Risk</strong> <span style="font-size:0.8rem;color:#8892b0;">&lt;40%</span></div>
    """, unsafe_allow_html=True)
...
if predict_btn:
    try:
        ...
        if probability >= 0.7:
            risk_class = "risk-high"; risk_emoji = "🔴"; risk_text = "High Risk"; risk_color = "#ff6b6b"
            bg_color = "linear-gradient(135deg, #1a0a0a, #2a0a0a)"; border_color = "#ff6b6b"
            risk_description = "Customer is very likely to churn. Immediate action required!"
        elif probability >= 0.4:
            risk_class = "risk-medium"; risk_emoji = "🟡"; risk_text = "Medium Risk"; risk_color = "#ff9f43"
            bg_color = "linear-gradient(135deg, #1a150a, #2a1a0a)"; border_color = "#ff9f43"
            risk_description = "Customer shows signs of potential churn. Proactive engagement needed."
        else:
            risk_class = "risk-low"; risk_emoji = "🟢"; risk_text = "Low Risk"; risk_color = "#4a6cf7"
            bg_color = "linear-gradient(135deg, #0a1a0a, #0a2a0a)"; border_color = "#4a6cf7"
        ...
        <div style="background:{bg_color}; padding:1.5rem; border-radius:1rem; border:2px solid {border_color}; margin-bottom:1rem;">
            <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap;">
                <div>
                    ...
                    <div style="font-size:2.5rem; font-weight:700; color:{risk_color};">{risk_emoji} {risk_text}</div>
                ...
                <div style="text-align:center;">
                    ...
                    <div style="font-size:1.5rem; font-weight:700; color:{'#ff6b6b' if prediction == 1 else '#4a6cf7'};">{risk_emoji} {'Will Churn' if prediction == 1 else 'Will Stay'}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            <div class="prediction-card" style="background:{bg_color}; border-color:{border_color};">
                ...
                <div class="prediction-value" style="color:{'#ff6b6b' if prediction == 1 else '#4a6cf7'};">{risk_emoji} {'⚠️ Churn' if prediction == 1 else '✅ No Churn'}</div>
            </div>
            """, unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
        ...
        with col3:
            ...
            <div class="prediction-card" style="background:{bg_color}; border-color:{border_color};">
                ...
                <div class="prediction-value {risk_class}">{risk_emoji} {risk_text}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if monthly_charges > 80:
            risk_factors.append("⚠️ High monthly charges (> ) - increases risk")
        elif monthly_charges > 60:
            risk_factors.append("ℹ️ Moderate charges (-80) - medium risk")
        else:
            risk_factors.append("✅ Low monthly charges (< ) - low risk")
